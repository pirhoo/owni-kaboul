$(function(){
	...
	$('#nav .ico1').click( function(){
		$('#fond').fadeTo('slow', 0.2, function() {
			$('#fond').fadeTo('slow', 1).css('background', 'url(img/map_dessin.jpg)')
		});
	});
	//NAV BTN IMAGE
		...
		$.ajax({
			...
			success: function(xml) {
				...
				$(node).find('pic').each( function() {
					pics += "<li><div id='delay'>"+$(this).attr('temp')+"</div><img height='400' src='data/"+id+"/img/"+$(this).attr('src')+"' /></li>";
				});
				...
				$('#nextBtn a').livequery( function(){
					$('#slider li').each(function(){ 
						var delay = $(this).children('#delay').text() * 100;
						//alert(delay);
